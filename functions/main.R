...
#  Description:
#' HARMONIES is a hybrid model to infer the microbiome network based on a sparse estimation 
#' of the precision matrix. The model consists of three major components: (1) a zero-inflated
#' negative binomial model for microbiome count data normalization; (2) the graphical lasso 
#' algorithm to obtain a sparse estimation of the precision matrix of the taxonomice features;
#' (3) a stability-based approach to select the tuning parameter for the graphical lasso.
...
#'                     Set the column name to be the name of taxa
...
# Outputs:
#' @return partial.corr A list of the partical correlation estimations for group 0 and group 1(if have two phenotypes) by HARMONIES
#' @return edge.estimation A list of data frames for group 0 and group 1(if have two phenotypes) containing the edge information of the estimated network 
#' @return node.estimation A data frame containing the node information of all the nodes shown in the estimated network(s)
...
# Output: 
#' @return FoldChange Fold change (group 1 over group 0) of the normalized abundance for all p taxa
...
#' @return pi An n-by-p matrix of the posterior mean of the missing probability pi for each count in the input matrix
...
# Descrip:
#' Apply Glasso with StARS method to learn a sparse network
